<template>
	<view>
		<div class="ticket">
			<div v-show="selectEle.id != ''">
				<div style="text-align: center;">该优惠码可用</div>
				<van-cell-group>
					<van-button @click="scanBillCode" type="primary" size="large">点击扫描单据二维码</van-button>
				</van-cell-group>
				<van-cell-group>
					<van-button @click="scanPosCode" type="primary" size="large">点击扫描POS小票二维码</van-button>
				</van-cell-group>
				<!-- <van-cell-group>
					<van-field :value="billCode" center clearable label="单据编号" placeholder="请输入录入单据编号" :border="false" use-button-slot>
						<van-button slot="button" size="small" type="primary">确定</van-button>
					</van-field>
				</van-cell-group>
				<van-cell-group>
					<van-field :value="posCode" center clearable label="POS小票" placeholder="请输入录入POS小票" :border="false"
					 use-button-slot>
						<van-button slot="button" size="small" type="primary">确定</van-button>
					</van-field>
				</van-cell-group> -->
			...
			scanBillCode() {
				...
				wx.scanCode({
					success(res) {
						...
						_this.$uniRequest.post('/api/small_ticket/check', {
							data: {
								cardId: _this.selectEle.cardId,
								code: _this.code,
								billCode: res.result
							}
						}).then(res => {
